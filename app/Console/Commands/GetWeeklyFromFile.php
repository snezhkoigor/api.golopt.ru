				    ...
				    foreach ($data_array as $row)
				    {
					    if ($row !== '')
					    {
						    ...
						    else
						    {
							    ...
							    $result[$pair][$expire][$row_array[0]]['option_type'] = OptionStrikes::TYPES_WEEK;
					    ...
					    foreach ($symbol as $expire_date)
					    {
						    ...
						    foreach ($expire_date as $strike)
						    {
						    	foreach ($strike['calls_puts'] as $option_type => $item)
							    {
							    	...
							    	$exists = DB::table(OptionStrikes::TYPES_WEEK . $week_number . '_option_parse')
									    ->where([
							    	...
							    	if (!$exists)
								    {
								    	DB::table(OptionStrikes::TYPES_WEEK . $week_number . '_option_parse')
										    ->insert($insert);
								    }
							    }
						    }
